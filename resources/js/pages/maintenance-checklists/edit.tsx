...
type Template = {
    ...
    items: { id: number; title: string; is_required: boolean; sort_order: number }[];
};
...
export default function MaintenanceChecklistEdit({ template, meta }: Props) {
    const { t } = useTranslation();
    const [items, setItems] = useState<Item[]>(
        (template.items || []).map((i) => ({ id: i.id, title: i.title, is_required: i.is_required, sort_order: i.sort_order })),
    );

    const form = useForm({
        name: template.name,
        asset_category_id: template.asset_category_id ? String(template.asset_category_id) : '',
    ...
    function submit() {
        form.patch(MaintenanceChecklistController.update.url({ template: template.id }), { preserveScroll: true });
    }

    return (
        <>
            <Head title={t('maintenance_checklists.actions.edit')} />

            <div className="flex h-full flex-1 flex-col gap-6 rounded-xl px-4 py-4 sm:px-6 sm:py-6">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    <Heading variant="small" title={t('maintenance_checklists.actions.edit')} description={template.name} />
                ...
                <div className="space-y-3">
                    ...
                    <div className="space-y-2">
                        {items.map((item, idx) => (
                            <div key={item.id ?? idx} className="flex flex-col gap-2 rounded-lg border p-3 sm:flex-row sm:items-center">
                                <Input
                                    value={item.title}
                                    onChange={(e) => {
                                        const next = [...items];

...
export default function AssetCategoriesIndex({ items }: Props) {
    ...
    const columns: DataTableColumn<Row>[] = [
        ...
        { key: 'code', label: t('asset_categories.fields.code'), sortable: true },
        {
            key: 'parent',
            label: t('asset_categories.fields.parent'),
            sortable: false,
            render: (row) =>
                row.parent ? (
    ...
    const rowActions: RowAction<Row>[] = [
        {
            ...
            onClick: (row) => router.visit(AssetCategoryController.edit.url({ asset_category: row.id })),
        ...
        },
        {
            ...
            variant: 'destructive',
            onClick: (row) => {
                if (confirm(t('asset_categories.actions.delete_confirm', { name: row.name }))) {
                    router.delete(AssetCategoryController.destroy.url({ asset_category: row.id }));
                }
            },
        ...
        <>
            ...
            <div className="flex h-full flex-1 flex-col gap-6 rounded-xl px-4 py-4 sm:px-6 sm:py-6">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <Heading variant="small" title={t('asset_categories.title')} description={t('asset_categories.description')} />

                    {abilities.create ? (
                        <Button onClick={() => router.visit(AssetCategoryController.create.url())}>
                            <Plus className="mr-2 h-4 w-4" />
                            {t('asset_categories.actions.new')}
                        </Button>
                    ) : null}
                </div>

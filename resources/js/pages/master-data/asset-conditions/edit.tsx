...
export default function AssetConditionEdit({ item }: Props) {
    ...
    function submit(e: React.FormEvent) {
        e.preventDefault();
        form.put(AssetConditionController.update.url({ asset_condition: item.id }));
    }

    return (
        <>
            <Head title={t('asset_conditions.actions.edit', { name: item.name })} />

            <div className="flex h-full flex-1 flex-col gap-6 rounded-xl px-4 py-4 sm:px-6 sm:py-6">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <Heading
                        variant="small"
                        title={t('asset_conditions.actions.edit', { name: item.name })}

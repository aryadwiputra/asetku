...
export default function CreateAssetLocation({ parents }: Props) {
    const { t } = useTranslation();

    return (
        <>
            <Head title={t('asset_locations.create.title')} />

            <div className="flex h-full flex-1 flex-col gap-6 rounded-xl px-4 py-4 sm:px-6 sm:py-6">
                <Heading variant="small" title={t('asset_locations.create.title')} description={t('asset_locations.create.description')} />

                <Form {...toForm(AssetLocationController.store())} className="max-w-2xl space-y-6">
                    {({ processing, errors }) => (
                        <>
...
CreateAssetLocation.layout = {
    breadcrumbs: [
        { title: 'common.master_data', href: masterDataIndex() },
        { title: 'asset_locations.title', href: assetLocationsIndex() },
        { title: 'asset_locations.actions.new', href: AssetLocationController.create.url() },
    ],
};

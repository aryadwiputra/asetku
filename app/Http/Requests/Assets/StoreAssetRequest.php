    ...
    public function rules(): array
    {
        ...
        return [
            'code' => [
                ...
                'string',
                'max:100',
                Rule::unique('assets', 'code')->where(fn ($query) => $query->where('organization_id', $organizationId)),
            ],
            ...
            'serial_number' => [
                ...
                'string',
                'max:255',
                Rule::unique('assets', 'serial_number')->where(fn ($query) => $query->where('organization_id', $organizationId)),
            ],
            'imei' => [
                'nullable',
                'string',
                'max:255',
                Rule::unique('assets', 'imei')->where(fn ($query) => $query->where('organization_id', $organizationId)),
            ],
